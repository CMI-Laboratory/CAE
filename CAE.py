...
from .utils_model import initialize_weights
...
class res_encoder(nn.Module):
    def __init__(self, f_num, f_size, input_size, dr=0.3, activation=nn.LeakyReLU(), channel_list=None):
        ...
        if channel_list is None: # (input channels,output channels, 0:유지 1:반토막, filter size)
            self.channel_list = [
...
class fully_res_VAE(nn.Module):
    ...
    def forward(self, input):
        ...
        return recon_out, (latent, torch.zeros_like(mu)), (mu, std)